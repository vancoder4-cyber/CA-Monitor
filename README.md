...
- **临近提醒**:正式事件进入距除息/生效日 **30 天**窗口知会一次，15–29 天安静，**≤14 天每天**催办一次（一天三次扫描也只推一次），7/3/1 天升级运营文案；单源预测沿用 30/14 天节奏，但只推数据核验并明确「勿执行」
...
之后每次 `python run.py` 跑完会自动把**正式催办 / 单源核验 / 新发现 / 冲突 / 空缺 / 预测状态更新**整理成一张交互卡片推到群里(filing 带 SEC 原文链接,底部「打开面板」按钮)。`state.json` 去重：30 天窗口首次知会一次，14 天内每天最多一次；单源条目明确标为核验且勿执行，升级、改期或失效也会推送。生产环境的 webhook 缺失或返回错误会直接让 Action 失败，且不推进去重状态，以便下次重试。单独测试推送:`python notify_lark.py`。
...
| 临近催办 | 临近催办 / 催办 / 临近 / 待执行 | 距除息/生效 0–14 天的正式催办 + 单源核验提醒；单源勿执行，与本周 7 天窗口分开 |
