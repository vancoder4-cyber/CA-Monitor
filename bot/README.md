...
| 风险 / 今日 / 本周 | 风控清单；本周按未来 7 个自然日内的除息/生效日统计正式事件并去重 |
| 新公告 / 临近催办 | 最近公司宣告；临近页列距除息/生效 0–14 天的正式催办和单源核验提醒，不代表本周清单；单源勿执行 |
...
4. `查 AAPL` 应返回当前支持范围内的单标的卡片；如有分红事件，卡片应同时显示官方入口与 StockAnalysis 交叉核对链接。正式事件不得带「单源待核实·勿执行」；单源事件只能显示核验提醒，不能出现正式运营执行文案。
