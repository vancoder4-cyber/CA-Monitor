...
# ADR 预扣税:外国公司派息时母国会在源头预扣税,存托行把**净额**转给 ADR 持有人。
# 我们认**毛额(税前)** —— 公司实际宣告的原值。此表用于:
#   ① 识别哪个源报的是净额(净额 ≈ 毛额×(1−税率),明显低于其它源);② 给运营提示别拿净额去确认。
# 税率是「法定/条约近似」,实际存托行可能扣更多(叠手续费);**检测不靠精确税率**(靠"毛额=各源最大值"),
# 税率只用于提示文案里的"已扣约 N%"。派息且有预扣税的 ADR 才列;美股无外国预扣税不列。
ADR_WHT = {
    "TSM":  ("台湾", 0.21),
    "ASML": ("荷兰", 0.15),
    "NVO":  ("丹麦", 0.27),
    ...
    "BB":   ("加拿大", 0.15),   # 加拿大条约 15%(目前不派息)
    "BSP":  ("巴西", 0.0),      # 巴西 JCP/股息机制特殊,暂记 0,派息前需复核
}
