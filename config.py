...
# 可仅凭 Item 代码确定为结构性公司行动的 8-K Item。
#
# 1.01（重大协议）、2.02（财报）、5.02（高管变动）、5.07（股东投票）、
# 7.01 和 8.01 都是宽泛披露容器，单靠 Item 号无法证明是公司行动。把它们
# 直接视为 CA 会导致财报/融资/任免被误报成「下架或暂停交易」。这些文件
# 仍保留在 SEC 原文表；真实的分红、并购、改名等由专用数据源、明确
# 结构性 Item 或人工官方覆盖进入公司行动流。
SEC_8K_CA_ITEMS = {"1.03", "2.01", "3.01", "3.03", "5.01"}
